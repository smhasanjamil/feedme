...
export const useCart = () => {
  ...
  const [removeItemApi, { isLoading: isRemoving }] =
    useRemoveFromCartMutation();
  const [removeItemByEmailApi, { isLoading: isRemovingByEmail }] =
    useRemoveFromCartByEmailMutation();
  const [updateQuantityApi, { isLoading: isUpdating }] =
    useUpdateCartItemMutation();
  ...
  const removeItem = async (itemId: string) => {
    if (!user?.email || !itemId) {
      console.error("Missing required parameters for item removal", {
        itemId,
        email: user?.email,
      });

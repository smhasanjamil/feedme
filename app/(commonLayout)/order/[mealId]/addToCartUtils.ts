...
export const getMealImageUrl = (meal: {
  imageUrl?: string;
  image?: string;
}): string | undefined => {
...
export const useAddToCart = () => {
  ...
  const addToCart = async (
    ...
    if (user.role === "provider" || user.role === "admin") {
      toast.error(
        `${user.role === "provider" ? "Providers" : "Admins"} cannot add items to cart`,
      );
      return false;
    }
      ...
      if (customization) {
        normalizedCustomization = {
          ...
          spiceLevel: customization.spiceLevel
            ? customization.spiceLevel.toLowerCase()
            : undefined,
        ...
        if (normalizedCustomization) {
          Object.keys(normalizedCustomization).forEach((key) => {
            if (
              normalizedCustomization &&
              normalizedCustomization[key as keyof CustomizationOptions] ===
                undefined
            ) {
